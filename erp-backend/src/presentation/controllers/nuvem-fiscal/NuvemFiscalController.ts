...
import { OAuth2Config } from '../../../domain/entities/nuvem-fiscal/NuvemFiscalTypes';
import { NuvemFiscalService } from '../../../infrastructure/external-apis/nuvem-fiscal/NuvemFiscalService';

// Use Cases - CNPJ
import { ConsultarCNPJUseCase } from '../../../domain/use-cases/nuvem-fiscal/ConsultarCNPJ';
...
} from '../../../domain/use-cases/nuvem-fiscal/GerenciarEmpresa';
...
} from '../../../domain/use-cases/nuvem-fiscal/GerenciarCertificado';
...
import {
  ...
  configuracaoDCeSchema,
  configuracaoDistribuicaoNFeSchema,
} from '../../validators/nuvem-fiscal/schemas';

/**
 * Controller para gerenciar todas as operações da Nuvem Fiscal
 */
export class NuvemFiscalController {
  ...
  async deletarEmpresa(c: Context) {
    try {
      ...
      return c.json(resultado, resultado.success ? 204 : this.getStatusCode(resultado.error));
    } catch (error: any) {
  ...
  async alterarConfiguracaoCTe(c: Context) {
    try {
      ...
      const resultado = await this.nuvemFiscalService.alterarConfiguracaoCTe(cpf_cnpj, validacao.data);
      return c.json(resultado, resultado.success ? 200 : this.getStatusCode(resultado.error));
    } catch (error: any) {
  ...
  private getStatusCode(error?: string): number {
    if (!error) return 500;
    if (error.includes('inválido')) return 400;
    if (error.includes('não encontrado') || error.includes('não autenticado')) return 404;
    if (error.includes('Não autenticado')) return 401;
