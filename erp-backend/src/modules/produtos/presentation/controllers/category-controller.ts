...
export class CategoryController {
  ...
  async list(c: Context<HonoContext>) {
    try {
      ...
      return c.json({ success: false, error: error.message || 'Failed to list categories' }, 400);
    }
  }
  ...
  async getById(c: Context<HonoContext>) {
    try {
      ...
      return c.json({ success: false, error: error.message || 'Failed to get category' }, 400);
    }
  }
  ...
  async create(c: Context<HonoContext>) {
    try {
      ...
      return c.json({ success: false, error: error.message || 'Failed to create category' }, 400);
    }
  }
  ...
  async update(c: Context<HonoContext>) {
    try {
      ...
      return c.json({ success: false, error: error.message || 'Failed to update category' }, 400);
    }
  }
  ...
  async remove(c: Context<HonoContext>) {
    try {
      ...
      return c.json({ success: true, message: 'Category deactivated successfully' });
    } catch (error: any) {
      return c.json({ success: false, error: error.message || 'Failed to delete category' }, 400);
    }
  }
}
