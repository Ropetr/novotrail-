...
export class PartnerController {
  ...
  async list(c: Context<HonoContext>) {
    try {
      ...
      return c.json({ success: false, error: error.message || 'Failed to list partners' }, 400);
    }
  }
  ...
  async getById(c: Context<HonoContext>) {
    try {
      ...
      return c.json({ success: false, error: error.message || 'Failed to get partner' }, 400);
    }
  }
  ...
  async create(c: Context<HonoContext>) {
    try {
      ...
      return c.json({ success: false, error: error.message || 'Failed to create partner' }, 400);
    }
  }
  ...
  async update(c: Context<HonoContext>) {
    try {
      ...
      return c.json({ success: false, error: error.message || 'Failed to update partner' }, 400);
    }
  }
  ...
  async remove(c: Context<HonoContext>) {
    try {
      ...
      return c.json({ success: false, error: error.message || 'Failed to delete partner' }, 400);
    }
  }
}
