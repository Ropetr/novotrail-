...
export class EmployeeController {
  ...
  async list(c: Context<HonoContext>) {
    try {
      ...
      return c.json({ success: false, error: error.message || 'Failed to list employees' }, 400);
    }
  }
  ...
  async getById(c: Context<HonoContext>) {
    try {
      ...
      return c.json({ success: false, error: error.message || 'Failed to get employee' }, 400);
    }
  }
  ...
  async create(c: Context<HonoContext>) {
    try {
      ...
      return c.json({ success: false, error: error.message || 'Failed to create employee' }, 400);
    }
  }
  ...
  async update(c: Context<HonoContext>) {
    try {
      ...
      return c.json({ success: false, error: error.message || 'Failed to update employee' }, 400);
    }
  }
  ...
  async remove(c: Context<HonoContext>) {
    try {
      ...
      return c.json({ success: true, message: 'Employee deactivated successfully' });
    } catch (error: any) {
      return c.json({ success: false, error: error.message || 'Failed to delete employee' }, 400);
    }
  }
}
