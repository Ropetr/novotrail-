 ...
 * Baseado na documentação v2.70.0
 */
...
export interface PaginacaoRequest {
  $top?: number; // Limite entre 1 e 100 (padrão: 10)
  $skip?: number; // Número de registros a ignorar (padrão: 0)
  $inlinecount?: boolean; // Incluir total de registros (padrão: false)
}
...
export interface ListarEmpresasRequest extends PaginacaoRequest {
  cpf_cnpj?: string; // Filtrar por CPF/CNPJ (sem máscara)
  nome_razao_social?: string; // Filtrar por nome/razão social (início do texto)
}
...
export interface ListarCertificadosRequest extends PaginacaoRequest {
  expires_in?: number; // Filtrar por dias até expiração (ex: 30, 7)
  include_expired?: boolean; // Incluir certificados vencidos (padrão: true)
}
...
export interface ConfiguracaoCTe {
  CRT: RegimeTributario; // Padrão: 3 (Regime Normal)
...
export interface ConfiguracaoDistribuicaoNFe {
  distribuicao_automatica?: boolean | null; // Padrão: false
  distribuicao_intervalo_horas?: number | null; // 1-24, Padrão: 24
  ciencia_automatica?: boolean | null; // Padrão: false
  ambiente: AmbienteNuvemFiscal;
}
...
export interface ConfiguracaoNFSe {
  ambiente: AmbienteNuvemFiscal;
  provedor?: string; // Provedor do município
}
